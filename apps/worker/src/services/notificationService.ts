...
export async function notifyUser(db: D1Database, opts: NotifyOpts): Promise<void> {
 ...
 */
export async function notifyUserDedup(db: D1Database, opts: NotifyOpts): Promise<void> {
  if (!(await typeEnabled(db, opts.recipientId, opts.type))) return
  try {
